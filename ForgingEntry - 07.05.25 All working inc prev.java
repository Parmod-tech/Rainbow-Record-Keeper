...
static JTextField textfentry, textfshift, textfqty, textfshifthrs, textheatcode;
static JFormattedTextField fdate;
static JComboBox<String> combopress, combopart;
static int fentryid;
...
//Frame and panels
		frame1 = new JFrame("BSFL (P-10): Forging Entry");
...
//Heat Code
		...
		panel1.add(HeatCode);
		textheatcode = new JTextField(4);
		panel1.add(textheatcode);
		...
		btnAdd.addActionListener(new ActionListener()
			...
			public void actionPerformed(ActionEvent e)
				...
				try
					...
					String queryadd="Insert into entry_prod (PressID, Shift, Part_ID, ProdQty, ShiftHrs, prod_Date, Heat_Code) Values (?, ?, ?, ?, ?, ?, ?)";
		...
		btnChange.addActionListener(new ActionListener()
			...
			public void actionPerformed(ActionEvent e)
				...
				try
					...
					String querychange="update entry_prod set PressID=?, Shift=?, Part_ID=?, Heat_Code=?, ProdQty=?, ShiftHrs=?, prod_Date=? where EntryID=?";
					...
					psmt.setInt(4, heatcode);
					
					psmt.setInt(5, fqty);
					psmt.setInt(6, shifthrs);
					psmt.setDate(7, sqlfdt);
					...
					psmt.close();
					dbclose();
					JOptionPane.showMessageDialog(frame1,"Entry ID: "+idnum+"  Changed Successfully!");
					}
				catch(Exception e1)
					{
					System.out.println(e1);
		...
		public void actionPerformed(ActionEvent e)
			{
				...
				{
				dbcon();
				String qryfind=" SELECT * FROM combined_prod where EntryID=?";
				psmt=con.prepareStatement(qryfind);
				psmt.setInt(1,idnum);
				rs=psmt.executeQuery();
				while(rs.next())
				{
				textfentry.setText(Integer.toString(rs.getInt(1)));
				textheatcode.setText(Integer.toString(rs.getInt(6)));
				
				java.sql.Date fdt1=(java.sql.Date) rs.getDate(8);
				long ldt=fdt1.getTime();
				java.util.Date retriveddt= new java.util.Date(ldt);
				fdate.setValue(retriveddt);
				
								
				combopress.setSelectedItem(rs.getString(9));
				textfshift.setText(rs.getString(3));
				textfshifthrs.setText(Integer.toString(rs.getInt(5)));
				combopart.setSelectedItem(rs.getString(10));

				textfqty.setText(Integer.toString(rs.getInt(7)));
				//ComboHeat.setSelectedItem(rs.getString(11));
...
static void getPrevious()
{
	try
		{
			dbcon();
			String queryprevious="SELECT EntryID, Part_Name, Heat_Code, Press_Code FROM pkdb.combined_prod where EntryId= (SELECT max(EntryID) from combined_prod where Press_Code= ? )";
			psmt=con.prepareStatement(queryprevious);
			psmt.setString(1, pressname);
			rs=psmt.executeQuery();
			
			while (rs.next())
			{
			combopart.setSelectedItem(rs.getString(2));
			textheatcode.setText(Integer.toString(rs.getInt(3)));
			}
...
static void getValues()
{
	...
	heatcode=Integer.parseInt(textheatcode.getText());
...
{	
	textfshift.setText("");
	textfshifthrs.setText("");
	textfqty.setText("");
	textheatcode.setText("");
}
