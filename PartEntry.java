...
//Button Find
		...
		JButton btnFindP = new JButton("Part Details");
		panel2.add(btnFindP);
		
		btnFindP.addActionListener(FindPartInfo);

//Button Change
		
		JButton btnChangeD = new JButton("Change Details");
		panel2.add(btnChangeD);
		
		btnChangeD.addActionListener(ActionChange);
...
ActionListener addPart = new ActionListener()
			...
			//JOptionPane.showMessageDialog(frame1,"custid="+custid+ " matid="+matid+ "secid=" +secid+ "found Successfully!");
...
//FindPartInfo----------------------------------------------------------
ActionListener FindPartInfo = new ActionListener()
			{
			@Override
			public void actionPerformed(ActionEvent e)
				{	
				...
				{
				dbcon();
				String querypartinfo="SELECT Part_ID, Part_Name, Std_CutWt, cust_name, mat_Grade, Part_No, Section, AsForged_wt, Remarks FROM pkdb.partlist_withdetails where Part_ID=?";
				psmt=con.prepareStatement(querypartinfo);
				psmt.setInt(1, Integer.parseInt(txtpartid.getText()));
				rs=psmt.executeQuery();
				while (rs.next())
				{
				txtpartid.setText(Integer.toString(rs.getInt(1)));
				txtpartname.setText(rs.getString(2));
				txtcutwt.setText(rs.getString(3));
				txtpartnumber.setText(rs.getString(6));
				txtforgwt.setText(rs.getString(8));
				txtremarks.setText(rs.getString(9));

				combocustname.setSelectedItem(rs.getString(4));
				combostdmat.setSelectedItem(rs.getString(5));
				combosection.setSelectedItem(rs.getString(7));
